...
void task_pq_fft_256(void)
{
    ...
    /* ź. */
    for (i = 0; i < APP_PQ_FFT_SAMPLE_COUNT_256; i++)
    {
        /* ʵ */
    ...
    for (i = 0u; (i < APP_PQ_FFT_SAMPLE_COUNT_256) && (i < LCD_WIDTH); i++)
    {
        gLcdWaveformDispBuf[i] = (uint16_t)(gPQFftF32In[i*2] * 128.0f);
    }

    /* PowerquadִFFTʱֻܴ */
    arm_float_to_q31(gPQFftF32In, gPQFftQ31In, APP_PQ_FFT_SAMPLE_COUNT_256*2u);

    /* cfftʹԭش.ԭźű,żź */
    for (i = 0u; i < APP_PQ_FFT_SAMPLE_COUNT_256 * 2u; i++)
    {
        gPQFftQ31InOut[i] = gPQFftQ31In[i] >> 5u; /* powerquadfftֻܴ27bit,ʱһ,ỹԭ */
    }

    /* ʼ㲢Թؼʱ */
    instance.fftLen = APP_PQ_FFT_SAMPLE_COUNT_256;/* ָFFT(ĸ) */
    TimerCount_Start();
    arm_cfft_q31(&instance, gPQFftQ31InOut, 0, 1); /*  */
    
    //PQ_TransformCFFT(POWERQUAD, APP_PQ_FFT_SAMPLE_COUNT_256, gPQFftQ31InOut, gPQFftQ31InOut);
    ...
    /* תĸ */
    arm_q31_to_float(gPQFftQ31Out, gPQFftF32Out, APP_PQ_FFT_SAMPLE_COUNT_256*2u);

    /* ԸҶϵȡģ,ʾӦƵϵ */
    arm_cmplx_mag_f32( gPQFftF32Out, gPQFftF32In, APP_PQ_FFT_SAMPLE_COUNT_256);/* ʱһgPQFftF32InĴռ */

    /* ӡƵ׵Դ */
    memset(gLcdFreqSpecDispBuf, 0u, sizeof(gLcdFreqSpecDispBuf));
    for (i = 0u; i < APP_PQ_FFT_SAMPLE_COUNT_256; i++)
    {
    ...
    /* ӡʱ */
    LCD_PrintWaveform(gLcdWaveformDispBuf, APP_PQ_FFT_SAMPLE_COUNT_256, LCD_COLOR_BLUE);
    sprintf((char *)gLcdTextDispBuf, "Input:");
    LCD_PrintWaveformText(gLcdTextDispBuf, LCD_COLOR_BLACK);

    /* ӡƵ */
    LCD_PrintFreqSpec(gLcdFreqSpecDispBuf, APP_PQ_FFT_SAMPLE_COUNT_256, LCD_COLOR_RED);
    sprintf((char *)gLcdTextDispBuf, "Output:");
    LCD_PrintFreqSpecText(gLcdTextDispBuf, LCD_COLOR_BLACK);

    /* ӡ */
    sprintf((char *)gLcdTextDispBuf, "PowerQuad FFT %d points", APP_PQ_FFT_SAMPLE_COUNT_256);
    ...
    gPQProcCycles[APP_USER_TASK_FFT_256_IDX] = calcTime;
}
